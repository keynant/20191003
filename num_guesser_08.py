...
class GameWindow:
    ...
    def check_entry(self):
        ...
        if not self.entry_box.get().isdecimal():
            ...
            self.name_fm = Frame(self.root)
            self.name_var = StringVar()
            self.name_var.set('Please enter your name')
            self.name_entry = Entry(self.name_fm, textvariable = self.name_var, font=('david', 12))
            self.name_entry.pack()
            self.name_entry.bind('<Button-1>', (lambda event: self.name_entry.delete(0,'end')))
            self.name_fm.pack()
            self.name_entry.bind('<Return>', (lambda event: update_hiscore(self.name_entry.get(), self.guesses.get())))
...
def update_hiscore(name, hiscore):
    search_add_name_hiscore(name, hiscore_list)

    if hiscore_list[name] < hiscore:
        hiscore_list[name] = hiscore
        with open("c:/NumGuesser/HiScore_list.txt", 'w') as f1:
            print("score upadted")
            json.dump(hiscore_list, f1)
    messagebox.showinfo('High score list', game_window.print_scoreboard())
